...
# Task 1: Go straight until an object is near, then turn right and continue until hitting a wall
def task1(rob: IRobobo, repetitions: int):
    ...
    for _ in range(repetitions):
        while True:
            irs = rob.read_irs()
            ir_data.append(irs)
            print(f"Task 1 IR Sensor Data: {irs}")
            if min(irs) < 50:  # Threshold for detecting an object near
                rob.move(0, 50, 1000)  # Turn right for 1 second
                rob.sleep(1)  # Allow some time to turn
                while True:
                    irs = rob.read_irs()
                    ir_data.append(irs)
                    if min(irs) < 10:  # Threshold for hitting the wall
                        rob.move(0, 0, 0)  # Stop
                        rob.sleep(1)
                        break
                    rob.move(50, 50, 100)  # Continue moving straight
                    rob.sleep(0.1)
                break
...
def task2(rob: IRobobo, repetitions: int):
    ...
    for _ in range(repetitions):
        while True:
            irs = rob.read_irs()
            ir_data.append(irs)
            print(f"Task 2 IR Sensor Data: {irs}")
            if min(irs) < 10:  # Threshold for touching the wall
                rob.move(-50, -50, 2000)  
                rob.sleep(5)  # Move backward for 5 seconds
                break
            rob.move(50, 50, 100)  # Move straight for 0.1 seconds
            rob.sleep(0.1)
...
def run_all_actions(rob: IRobobo):
    repetitions = 10  # Define the number of repetitions for each task
    ...
    plot_sensor_data(ir_data_task1, 'IR Sensor Data for Task 1')
    
    print("Running Task 2")
    # Example Task 2
    ir_data_task2 = task2(rob, repetitions)
    plot_sensor_data(ir_data_task2, 'IR Sensor Data for Task 2')
